...
public class CalendarController {
    ...
    public static final int MAX_CALENDAR_WEEK = 3549; // weeks between 1/1/1970
                                                      // and 1/1/2038
    /* @} */

    /* UNISOC: Modify for bug 1071752 @{ */
    public static final int MIN_CALENDAR_MILLIS = -28800; // 1970/1/1 00:00:00
    /* @} */

    // SPRD: Add for bug569477, tip read contact.
    public static final String VISIT_CONTACT = "visit_contact";
    public AlertDialog mReadContactDialog;
    ...
    private final LinkedHashMap<Integer, EventHandler> eventHandlers = new LinkedHashMap<Integer, EventHandler>(
            5);
    ...
    private final WeakHashMap<Object, Long> filters = new WeakHashMap<Object, Long>(
            1);
    ...
    public static class EventInfo {
        ...
        public boolean isAllDay() {
            if (eventType != EventType.VIEW_EVENT) {
                /* SPRD: Modify for bug 680928 @{ */
                Log.d(TAG, "illegal call to isAllDay , wrong event type "
                        + eventType);
                /* @} */
                return false;
            }
        ...
        public int getResponse() {
            if (eventType != EventType.VIEW_EVENT) {
                /* SPRD: Modify for bug 680928 @{ */
                Log.d(TAG, "illegal call to getResponse , wrong event type "
                        + eventType);
                /* @} */
            ...
            default:
                /* SPRD: Modify for bug 680928 @{ */
                Log.d(TAG, "Unknown attendee response " + response);
                /* @} */
            }
    ...
    public static CalendarController getInstance(Context context) {
        synchronized (instances) {
            ...
            WeakReference<CalendarController> weakController = instances
                    .get(context);
    ...
    public void sendEventRelatedEvent(Object sender, long eventType,
            long eventId, long startMillis, long endMillis, int x, int y,
            long selectedMillis) {
        ...
        sendEventRelatedEventWithExtra(sender, eventType, eventId, startMillis,
                endMillis, x, y, EventInfo.buildViewExtraLong(
                        Attendees.ATTENDEE_STATUS_NONE, false), selectedMillis);
    }
     ...
     */
    public void sendEventRelatedEventWithExtra(Object sender, long eventType,
            long eventId, long startMillis, long endMillis, int x, int y,
            long extraLong, long selectedMillis) {
        sendEventRelatedEventWithExtraWithTitleWithCalendarId(sender,
                eventType, eventId, startMillis, endMillis, x, y, extraLong,
     ...
     */
    public void sendEventRelatedEventWithExtraWithTitleWithCalendarId(
            Object sender, long eventType, long eventId, long startMillis,
        ...
        if (eventType == EventType.EDIT_EVENT
                || eventType == EventType.VIEW_EVENT_DETAILS) {
        ...
        if (selectedMillis != -1) {
            info.selectedTime = new Time(Utils.getTimeZone(mContext,
                    mUpdateTimezone));
     ...
     *            {@link ViewType}
     */
    public void sendEvent(Object sender, long eventType, Time start, Time end,
            long eventId, int viewType) {
    ...
    public void sendEvent(Object sender, long eventType, Time start, Time end,
        ...
        sendEvent(sender, eventType, start, end, start, eventId, viewType,
                extraLong, query, componentName);
    }

    public void sendEvent(Object sender, long eventType, Time start, Time end,
            Time selected, long eventId, int viewType, long extraLong,
            String query, ComponentName componentName) {
        EventInfo info = new EventInfo();
        info.eventType = eventType;
    ...
    public void sendEvent(Object sender, final EventInfo event) {
        ...
        if (filteredTypes != null
                && (filteredTypes.longValue() & event.eventType) != 0) {
        ...
        if (DEBUG) {
            ...
            Log.d(TAG,
                    "Start  "
                            + (event.startTime == null ? "null"
                                    : event.startTime.toString()));
            Log.d(TAG,
                    "End    "
                            + (event.endTime == null ? "null" : event.endTime
                                    .toString()));
            Log.d(TAG,
                    "Select "
                            + (event.selectedTime == null ? "null"
                                    : event.selectedTime.toString()));
            Log.d(TAG, "mTime  " + (mTime == null ? "null" : mTime.toString()));
        }

        long startMillis = MIN_CALENDAR_MILLIS - 1; // UNISOC: Modify for bug 1071752
        ...
        /*
         * SPRD: bug474714 can not select 8:00 am when current timezone @{ is
         * +8(Beijing)
         */
        // if (event.selectedTime != null && event.selectedTime.toMillis(false)
        // != 0) {
        if (event.selectedTime != null) {
            mTime.set(event.selectedTime);
        } else {
            if (startMillis != MIN_CALENDAR_MILLIS - 1) { // UNISOC: Modify for bug 1071752
                ...
                if (mtimeMillis < startMillis
                        || (event.endTime != null && mtimeMillis > event.endTime
                                .toMillis(false))) {
        ...
        /*
         * SPRD for bug458035/501401/1071752，if date is 1970-1-1 and isAllDay is true,
         * don't set event.startTime = mTime. @}
         */
        /*boolean useSelectedTime = false;
        if (event != null && event.startTime != null && event.endTime != null) {
            useSelectedTime = event.isAllDay()
                    && event.startTime.toMillis(false) == 0
        ...
        synchronized (this) {
            ...
            if (DEBUG) {
                Log.d(TAG, "sendEvent: Dispatching to " + eventHandlers.size()
                        + " handlers");
            }
            ...
            if (mFirstEventHandler != null) {
                ...
                if (handler != null
                        && (handler.getSupportedEventTypes() & event.eventType) != 0
                        && !mToBeRemovedEventHandlers
                                .contains(mFirstEventHandler.first)) {
                    handler.handleEvent(event);
                    handled = true;
                }
            }
            for (Iterator<Entry<Integer, EventHandler>> handlers = eventHandlers
                    .entrySet().iterator(); handlers.hasNext();) {
                Entry<Integer, EventHandler> entry = handlers.next();
                int key = entry.getKey();
                if (mFirstEventHandler != null
                        && key == mFirstEventHandler.first) {
            ...
            if (mDispatchInProgressCounter == 0) {
                ...
                if (mToBeRemovedEventHandlers.size() > 0) {
                    for (Integer zombie : mToBeRemovedEventHandlers) {
                        ...
                        if (mFirstEventHandler != null
                                && zombie.equals(mFirstEventHandler.first)) {
                ...
                if (mToBeAddedEventHandlers.size() > 0) {
                    for (Entry<Integer, EventHandler> food : mToBeAddedEventHandlers
                            .entrySet()) {
        ...
        if (!handled) {
            ...
            if (event.eventType == EventType.CREATE_EVENT) {
                ...
                launchViewEvent(event.id, event.startTime.toMillis(false),
                        endTime, event.getResponse());
                return;
            } else if (event.eventType == EventType.EDIT_EVENT) {
                launchEditEvent(event.id, event.startTime.toMillis(false),
                        endTime, true);
                return;
                ...
                launchDeleteEvent(event.id, event.startTime.toMillis(false),
                        endTime);
                return;
    ...
    public void registerFirstEventHandler(int key, EventHandler eventHandler) {
        synchronized (this) {
            ...
            if (mDispatchInProgressCounter > 0) {
                mToBeAddedFirstEventHandler = new Pair<Integer, EventHandler>(
                        key, eventHandler);
            } else {
                mFirstEventHandler = new Pair<Integer, EventHandler>(key,
                        eventHandler);
            }
        }
    }
    ...
    public void deregisterEventHandler(Integer key) {
        synchronized (this) {
            if (mDispatchInProgressCounter > 0) {
                ...
                if (mFirstEventHandler != null
                        && key.equals(mFirstEventHandler.first)) {// UNISOC: Modify for bug1209634
                    mFirstEventHandler = null;
                }
            }
        }
    ...
    private void launchCreateEvent(long startMillis, long endMillis,
        ...
        final Intent intent = generateCreateEventIntent(startMillis, endMillis,
                allDayEvent, title, calendarId);
        mEventId = -1;
        /* SPRD: Add for bug569477, tip read contact. @{ */
        if ("1".equals(Utils.mSecureCheck) && needDisplayContactDialog()) {
            mReadContactDialog = new AlertDialog.Builder(mContext)
            ...
            /*
             * SPRD: Add for bug617340, IllegalArgumentException, Calendar Crash
             * in monkeys @{
             */
            /*
    ...
    public boolean isReadContactDialogShowing() {
        if (null != mReadContactDialog) {
            return mReadContactDialog.isShowing();
        }
        return false;
    }
    ...
    public void dismissContactDialog() {
        if (mReadContactDialog != null && mReadContactDialog.isShowing()) {
    ...
    private void launchEditEvent(long eventId, long startMillis,
            long endMillis, boolean edit) {
    ...
    private void launchDeleteEvent(long eventId, long startMillis,
            long endMillis) {
        launchDeleteEventAndFinish(null, eventId, startMillis, endMillis, -1);
    }

    private void launchDeleteEventAndFinish(Activity parentActivity,
            long eventId, long startMillis, long endMillis, int deleteWhich) {
        DeleteEventHelper deleteEventHelper = new DeleteEventHelper(mContext,
                parentActivity, parentActivity != null /* exit when done */);
        deleteEventHelper.delete(startMillis, endMillis, eventId, deleteWhich);
    }

    /*UNISOC: Modify for bug 1170703 @{*/
    private void launchSearch(long eventId, String query,
            ComponentName componentName) {
        componentName = new ComponentName(componentName.getPackageName(), SearchActivity.class.getName());
    ...
    private boolean needDisplayContactDialog() {
        SharedPreferences localPreferences = mContext.getSharedPreferences("local_shareference", Activity.MODE_PRIVATE);
