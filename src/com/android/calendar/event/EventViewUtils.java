...
public class EventViewUtils {
    ...
    // "2 hours".
    public static String constructReminderLabel(Context context, int minutes,
            boolean abbrev) {
     ...
     *
     * @param values
     *            the list of minutes corresponding to the spinner choices
     * @param minutes
     *            the minutes to search for in the values list
     * @return the index of "minutes" in the "values" list
     */
    public static int findMinutesInReminderList(ArrayList<Integer> values,
            int minutes) {
    ...
    /**
     * Finds the index of the given method in the "methods" list. If the method
     * isn't present (perhaps because we don't think it's allowed for this
     * calendar), we return zero (the first item in the list).
     * <p>
     * With the current definitions, this effectively converts DEFAULT and
     * unsupported method types to ALERT.
     *
     * @param values
     *            the list of minutes corresponding to the spinner choices
     * @param method
     *            the method to search for in the values list
     * @return the index of the method in the "values" list
     */
    public static int findMethodInReminderList(ArrayList<Integer> values,
            int method) {
        int index = values.indexOf(method);
        if (index == -1) {
            // If not allowed, or undefined, just use the first entry in the
            // list.
            // Log.d(TAG, "Cannot find method (" + method +
            // ") in allowed list");
            index = 0;
        }
        return index;
     ...
     *
     * @param reminderItems
     *            UI elements (layouts with spinners) that hold array indices.
     * @param reminderMinuteValues
     *            Maps array index to time in minutes.
     * @param reminderMethodValues
    ...
    public static ArrayList<ReminderEntry> reminderItemsToReminders(
        ...
        for (int index = 0; index < len; index++) {
            ...
            Spinner minuteSpinner = (Spinner) layout
                    .findViewById(R.id.reminder_minutes_value);
            Spinner methodSpinner = (Spinner) layout
                    .findViewById(R.id.reminder_method_value);
            int minutes = reminderMinuteValues.get(minuteSpinner
                    .getSelectedItemPosition());
            int method = reminderMethodValues.get(methodSpinner
                    .getSelectedItemPosition());
    ...
    /**
     * If "minutes" is not currently present in "values", we add an appropriate
     * new entry to values and labels.
     */
    public static void addMinutesToList(Context context,
            ArrayList<Integer> values, ArrayList<String> labels, int minutes) {
        int index = values.indexOf(minutes);
        if (index != -1) {
            return;
        }
    ...
    /**
     * Remove entries from the method list that aren't allowed for this
     * calendar.
     *
     * @param values
     *            List of known method values.
     * @param labels
     *            List of known method labels.
     * @param allowedMethods
     *            Has the form "0,1,3", indicating method constants from
     *            Reminders.
     */
    public static void reduceMethodList(ArrayList<Integer> values,
            ArrayList<String> labels, String allowedMethods) {
        ...
        // Walk through the method list, removing entries that aren't in the
        // allowed list.
     ...
     */
    private static void setReminderSpinnerLabels(Activity activity,
            Spinner spinner, ArrayList<String> labels) {
        Resources res = activity.getResources();
        spinner.setPrompt(res.getString(R.string.reminders_label));
        int resource = R.layout.reminder_item;
     ...
     * spinners in the reminder. If a listener is set it will store the initial
     * position of the spinner into the spinner's tag for comparison with any
    ...
    public static boolean addReminder(Activity activity, View view,
        ...
        LinearLayout reminderItem = (LinearLayout) inflater.inflate(
                R.layout.edit_reminder_item, null);
        parent.addView(reminderItem);

        ImageButton reminderRemoveButton;
        reminderRemoveButton = (ImageButton) reminderItem
                .findViewById(R.id.reminder_remove);
        reminderRemoveButton.setOnClickListener(listener);

        /*
         * The spinner has the default set of labels from the string resource
         * file, but we want to drop in our custom set of labels because it may
         * have additional entries.
         */
        Spinner spinner = (Spinner) reminderItem
                .findViewById(R.id.reminder_minutes_value);
        setReminderSpinnerLabels(activity, spinner, minuteLabels);

        int index = findMinutesInReminderList(minuteValues,
        ...
        /*
         * Configure the alert-method spinner. Methods not supported by the
         * current Calendar will not be shown.
         */
        spinner = (Spinner) reminderItem
                .findViewById(R.id.reminder_method_value);
    ...
    /**
     * Enables/disables the 'add reminder' button depending on the current
     * number of reminders.
     */
    public static void updateAddReminderButton(View view,
            ArrayList<LinearLayout> reminders, int maxReminders) {
