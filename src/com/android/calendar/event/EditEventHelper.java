...
public class EditEventHelper {
    ...
    void checkTimeDependentFields(CalendarEventModel originalModel, CalendarEventModel model,
        ...
        if (modifyWhich == MODIFY_ALL) {
            ...
            /* SPRD: for bug557761/581896, the offset has error in calculation. @{ */
            Time startTime = new Time(newTimezone);
            startTime.set(oldStartMillis);
            oldStartMillis = offsetOldStartMillisIfNecessary(startTime, newRrule);
            /* @} */
    ...
    private long offsetOldStartMillisIfNecessary(Time startTime, String rrule) {
        Log.d(TAG, "Before offset, startTime = " + startTime);
        long newStartMillis = 0;
        long oldStartMillis = startTime.normalize(true);
        if (rrule == null || rrule.isEmpty()) {
            // No need to waste any time with the parsing if the rule is empty.
            return oldStartMillis;
        }
        ...
        if (mEventRecurrence.freq == EventRecurrence.MONTHLY) {
            int weekth = (startTime.monthDay + 6) / 7;
            for (int i = 0; i < mEventRecurrence.bydayCount; i++) {
                int ruleWeekth = mEventRecurrence.bydayNum[i];
                int ruleWeek = EventRecurrence.day2TimeDay(mEventRecurrence.byday[i]);
                int weekOffset = ruleWeek - startTime.weekDay;
                if (weekth == ruleWeekth && startTime.weekDay == ruleWeek) {
                    return oldStartMillis;
                } else if (ruleWeekth == -1) {
                    int month = startTime.month;
                    if (startTime.weekDay != ruleWeek) {
                        startTime.monthDay = startTime.monthDay + (weekOffset > 0 ? weekOffset : weekOffset + 7);
                        startTime.normalize(true);
                        Log.d(TAG, "startTime = " + startTime);
                    }
                    while (startTime.month == month) {
                        startTime.monthDay = startTime.monthDay + 7;
                        startTime.normalize(true);
                    }
                    startTime.monthDay = startTime.monthDay - 7;
                } else {
                    if (startTime.weekDay != ruleWeek) {
                        startTime.monthDay = startTime.monthDay + weekOffset;
                    }
                    if (weekth != ruleWeekth) {
                        startTime.monthDay = startTime.monthDay + 7 * (ruleWeekth - weekth);
                    }
                }
            }

        } else if (mEventRecurrence.freq == EventRecurrence.WEEKLY) {
        ...
        newStartMillis = startTime.normalize(true);
        Log.d(TAG, "After offset, startTime = " + startTime);
