...
public class EditEventView implements View.OnClickListener,
    ...
    private boolean mVisitContact = true;
    private int[] mOriginalPadding = new int[4];

    public boolean mIsMultipane;
    private ProgressDialog mLoadingCalendarsDialog;
    public AlertDialog mNoCalendarsDialog;// UNISOC: Modify for bug1206642
    ...
    private TimePickerDialog mTimeDialog; // UNISOC: Modify for bug1232836
    ...
    private class TimeListener implements OnTimeSetListener {
        ...
        @Override
        public void onTimeSet(TimePicker view, int hourOfDay, int minute) { // UNISOC: Modify for bug1232836
            ...
            /*
             * SPRD: Add for bug442148/454220/463755/492933. Date beyond
             * 2037-12-31 @}
             */
            ...
            /* @} */
            /*
             * SPRD: Add for bug505589, to ensure that startMillis is less than
             * endMillis @}
             */
            /*
             * SPRD: Add for bug513282, the internal and external display is not
             * consistent for EndTime. @}
             */
        ...
        public void onClick(View v) {
            ...
            if (v == mStartTimeButton) {
                ...
                if (mStartTimePickerDialog == null) {
                    mStartTimePickerDialog = new TimePickerDialog(mActivity,
                            new TimeListener(v), mTime.hour, mTime.minute,
                            DateFormat.is24HourFormat(mActivity));
                } else {
                    mStartTimePickerDialog.updateTime(mTime.hour,
                            mTime.minute);
                }
                mTimeDialog = mStartTimePickerDialog;
            } else {
                mTimeSelectedWasStartTime = false;
                if (mEndTimePickerDialog == null) {
                    mEndTimePickerDialog = new TimePickerDialog(mActivity,
                            new TimeListener(v), mTime.hour, mTime.minute,
                            DateFormat.is24HourFormat(mActivity));
                } else {
                    mEndTimePickerDialog.updateTime(mTime.hour, mTime.minute);
                }
                mTimeDialog = mEndTimePickerDialog;
            }
    ...
    private void showTimezoneDialog() {
        ...
        b.putString(TimeZonePickerDialog.BUNDLE_TIME_ZONE, "Asia/Shanghai");
    ...
    private void populateRepeats() {
        ...
        if (!TextUtils.isEmpty(mRrule)) {
            ...
            if (enabled && Time.compare(mStartTime, mOldTIme) != 0
                    && !isRepeatDone) {
    ...
    private class DateClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (!ActivityManager.isUserAMonkey()) {
                ...
                datePicker.setFirstDayOfWeek(Utils.getFirstDayOfWeekAsCalendar(mActivity));
            ...
            if (name != null) {
                ...
                if (accountName != null) {
                    ...
                    if (ownerName.equals("Local Calendar")) {
                        ownerName = context.getResources().getString(
                                R.string.local_account);
                    }
    ...
    public void showRecurrencePickerDialog() {
        ...
        rpd.setStyle(1,R.style.RecurrencePickerStyle); // UNISOC: Modify for bug1227302,1232836
        rpd.setArguments(b);
    ...
    private boolean fillModelFromUI() {
        ...
        if (mAttendeesList != null) {
            ...
            if(mEtAttendeesList != null) {
                mModel.addAttendees(mEtAttendeesList.getText().toString(),
                        mEmailValidator);
            }
            mEmailValidator.setRemoveInvalid(false);
        }
        ...
        mModel.mAvailability = mAvailabilityValues.get(mAvailabilitySpinner
                .getSelectedItemPosition() >= mAvailabilityValues.size() ? 0 : mAvailabilitySpinner.getSelectedItemPosition());
        /*
         * SPRD: Add for bug452427/454220, Date super bound, cannot create event
         * @{
         */
        if (mModel.mAllDay) {
            if (mModel.mStart > Utils.END_DAY_MAX_ALLDAY
                    || mModel.mEnd > Utils.END_DAY_MAX_ALLDAY
                    || mModel.mEnd == -1) {
                Toast.makeText(
                        mActivity,
                        mActivity.getResources().getString(
                                R.string.not_save_event), Toast.LENGTH_SHORT)
                        .show();
                return false;
            }
        } else {
            if (mModel.mStart >= Utils.END_DAY_MAX
                    || mModel.mEnd >= Utils.END_DAY_MAX || mModel.mEnd == -1) {
                Toast.makeText(
                        mActivity,
                        mActivity.getResources().getString(
                                R.string.not_save_event), Toast.LENGTH_SHORT)
    ...
    public EditEventView(Activity activity, View view, EditDoneRunnable done,
            ...
            boolean visitContact, boolean isAvailabilityExplicitlySet) {
        // SPRD: Add for bug569477, tip read contact.
        mVisitContact = visitContact;
        mActivity = activity;
        mView = view;
        mDone = done;
        ...
        /* SPRD: Add for bug569477, tip read contact. @{ */
        if (!mVisitContact) {
            mEtAttendeesList = (EditText) view.findViewById(R.id.attendees_et);
            if(mEtAttendeesList != null) {
                mEtAttendeesList.setVisibility(View.VISIBLE);
            }
            if(mAttendeesList != null) {
                mAttendeesList.setVisibility(View.GONE);
                mAttendeesList = null;
            }            
        }
        /* @} */
                    ...
                    .setOnEditorActionListener(new OnEditorActionListener() {
                        ...
                        public boolean onEditorAction(TextView v, int actionId,
                            ...
                            if (actionId == EditorInfo.IME_ACTION_DONE) {
                                // Dismiss the suggestions dropdown. Return
                                // false so the other
        ...
        mAvailabilitySpinner
                .setOnItemSelectedListener(new OnItemSelectedListener() {
                    ...
                    public void onItemSelected(AdapterView<?> parent,
                        ...
                        // being
                        // initialized to the first item, and when we explicitly
                        // set it
                        // in the allDay checkbox toggling, so we need these
                        // checks to
        ...
        /*
         * if(mAttendeesList != null){
         * mAttendeesList.setTag(mAttendeesList.getBackground()); }else{
         * mEtAttendeesList.setTag(mEtAttendeesList.getBackground()); }
         */
        /* @} */
        ...
        } else if(mEtAttendeesList != null) {
            mEditViewList.add(mEtAttendeesList);
        }
        ...
        SprdTextWatcher titleTextWatcher = new SprdTextWatcher(mActivity,
                mTitleTextView, MESSAGE_SUBJECT_TEXT_MAX_LENGTH);
        mTitleTextView.addTextChangedListener(titleTextWatcher);
        SprdTextWatcher locationTextWatcher = new SprdTextWatcher(mActivity,
                mLocationTextView, MESSAGE_LOCATION_Text_MAX_LENGTH);
        ...
        SprdTextWatcher descriptionTextWatcher = new SprdTextWatcher(mActivity,
                mDescriptionTextView, MESSAGE_CONTENT_TEXT_MAX_LENGTH);
    ...
    private void prepareAvailability() {
        ...
        mAvailabilityAdapter = new CustomAdapter(mActivity, r.getStringArray(R.array.availability));
    ...
    private void prepareAccessLevel() {
        Resources r = mActivity.getResources();

        mAccessLevelSpinner.setAdapter(new CustomAdapter(mActivity, r.getStringArray(R.array.visibility)));
    }
    ...
    // SPRD: Add for bug569477, tip read contact.
    public void setModel(final CalendarEventModel model) {
        mModel = model;
        /* Bug #442566/474232 Add for Local Calendar bugs @{ */
        SharedPreferences localPreferences = mActivity.getSharedPreferences(
                LOCAL_NAME_PREFERENCES, Activity.MODE_PRIVATE);
        ...
        String displayName = TextUtils.isEmpty(model.mCalendarDisplayName) ? name
                : model.mCalendarDisplayName;
        /* @{ */
        if (canRespond && !displayName.equals("Local Calendar")) {
        ...
        if (model.mUri != null || !TextUtils.isEmpty(uri)) {
            ...
            /* Bug #442566/474232 Add for Local Calendar bugs @{ */
            if (displayName.equals("Local Calendar")) {
                displayName = mActivity.getResources().getString(
                        R.string.local_account);
                /*
                 * SPRD for bug 443435/453152/474232,if account is local
                 * account,let mAttendeesGroup dismiss @{
                 */
                if (mAttendeesList != null) {
            ...
            String ownerName = localPreferences
                    .getString("mOwnerAccount", null);
            if (tv != null) {
                /* Bug #442566/474232 Add for Local Calendar bugs @{ */
                String ownerAccount = TextUtils.isEmpty(model.mOwnerAccount) ? ownerName
                        : model.mOwnerAccount;
                if (ownerAccount.equals("Local Calendar")) {
                    ownerAccount = mActivity.getResources().getString(
                            R.string.local_account);
                }
    ...
    public void setCalendarsCursor(Cursor cursor, boolean userVisible,
        ...
        if (cursor == null || cursor.getCount() == 0) {
            ...
            if (mSaveAfterQueryComplete && mLoadingCalendarsDialog != null) { // UNISOC: Modify for bug1221478
    ...
    private void updateAttendees(HashMap<String, Attendee> attendeesList) {
        ...
        for (Attendee attendee : attendeesList.values()) {
            ...
            if (mAttendeesList != null) {
                ...
                mEtAttendeesList.append(attendee.mEmail + ", ");
            }
            /* @} */
        }
    }
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position,
        ...
        /*
         * SPRD for bug 443435/453152/455407/457832/463506/474232,if account is
         * local account,let mAttendeesGroup dismiss @{
         */
    ...
    public boolean checkEmailAddress() {
        ...
        if ("".equals(attendee)) {
            ...
            for (int i = 0; i < tokens.length; i++) {
                if (!EMAIL_ADDRESS_PATTERN.matcher(tokens[i].getAddress())
                        .matches()) {
                    result = false;
                }
            }
    ...
    /*
     * SPRD for bug446277 when EditEventFragment destroyed, EditEventView.mRrule
     * should to be null@{
     */
