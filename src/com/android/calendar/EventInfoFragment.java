...
public class EventInfoFragment extends DialogFragment implements
    ...
    /* SPRD: Add for bug569477, tip read contact. @{ */
    protected static final String BUNDLE_KEY_ISSHOWDIALOG = "key_is_show_dialog";
    protected static final String BUNDLE_KEY_ISSELECTFLAG = "key_is_select_flag";
    /* @{ */
    ...
    private static final String[] EVENT_PROJECTION = new String[] { Events._ID, // 0
            ...
            Events.DTEND, // 20
            Events.DURATION, // 21
            Events.ORIGINAL_SYNC_ID // 22 do not remove; used in
                                    // DeleteEventHelper
    };
    ...
    static {
        if (!Utils.isJellybeanOrLater()) {
            EVENT_PROJECTION[EVENT_INDEX_CUSTOM_APP_PACKAGE] = Events._ID; // dummy
                                                                           // value
            EVENT_PROJECTION[EVENT_INDEX_CUSTOM_APP_URI] = Events._ID; // dummy
                                                                       // value

            ATTENDEES_PROJECTION[ATTENDEES_INDEX_IDENTITY] = Attendees._ID; // dummy
                                                                            // value
            ATTENDEES_PROJECTION[ATTENDEES_INDEX_ID_NAMESPACE] = Attendees._ID; // dummy
                                                                                // value
        }
    }
    ...
    static final String[] COLORS_PROJECTION = new String[] { Colors._ID, // 0
            Colors.COLOR, // 1
            Colors.COLOR_KEY // 2
    };

    static final String COLORS_WHERE = Colors.ACCOUNT_NAME + "=? AND "
            + Colors.ACCOUNT_TYPE + "=? AND " + Colors.COLOR_TYPE + "="
            + Colors.TYPE_EVENT;
    ...
    // SPRD for bug455353, edit a action, if it has Attendees, toast should be
    // "Updates will be sent".
    private int mAttendeesSize;

    /* SPRD: Add for bug569477, tip read contact. @{ */
    private boolean mIsReadContact = true;
    private AlertDialog mReadContactDialog;
    public AlertDialog mReadUserDataDialog; // UNISOC: Modify for bug1206642
    private boolean mIsShowDialog = false;
    private boolean mIsSelectedFlag = true;
    /* @} */

    private int mEventInfoBgColor;// UNISOC: Modify for bug1227302
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            case TOKEN_QUERY_EVENT:
                ...
                if (!mCalendarColorInitialized) {
                    mCalendarColor = Utils
                            .getDisplayColorFromColor(mEventCursor
                                    .getInt(EVENT_INDEX_CALENDAR_COLOR));
                    mCalendarColorInitialized = true;
                }

                if (!mOriginalColorInitialized) {
                    mOriginalColor = mEventCursor
                            .isNull(EVENT_INDEX_EVENT_COLOR) ? mCalendarColor
                            : Utils.getDisplayColorFromColor(mEventCursor
                                    .getInt(EVENT_INDEX_EVENT_COLOR));
            ...
            case TOKEN_QUERY_COLORS:
                ...
                if (cursor.moveToFirst()) {
                    do {
                        ...
                        int displayColor = Utils
                                .getDisplayColorFromColor(rawColor);
                ...
                Arrays.sort(colors.toArray(sortedColors),
                        new HsvColorComparator());
            ...
            case TOKEN_QUERY_ATTENDEES:
                ...
                /*
                 * SPRD: Add for bug555195, do not display attendee info on
                 * "Local Calendar". @{
                 */
                String calendarNameStr = mCalendarsCursor
                        .getString(CALENDARS_INDEX_DISPLAY_NAME);
                if (calendarNameStr.equals("Local Calendar")) {
                    break;
                }
                /* @} */
            ...
            case TOKEN_QUERY_VISIBLE_CALENDARS:
                if (cursor.getCount() > 1) {
                    // Start duplicate calendars query to detect whether to add
                    // the calendar
                    ...
                    // Don't need to display the calendar owner when there is
                    // only a single
                    // calendar. Skip the duplicate calendars query.
                    String displayName = mCalendarsCursor
                            .getString(CALENDARS_INDEX_DISPLAY_NAME);
                    setVisibilityCommon(mView, R.id.calendar_container,
                            View.VISIBLE);
                    /* SPRD: Modify for bug 729869. Translation for local accounts @{ */
                    if (displayName.equals("Local Calendar")) {
                        displayName = (String) mActivity.getResources().getString(
            ...
            case TOKEN_QUERY_DUPLICATE_CALENDARS:
                ...
                String calendarName = mCalendarsCursor
                        .getString(CALENDARS_INDEX_DISPLAY_NAME);
                /*
                 * SPRD: Modify for bug448834/474232. Translation for local
                 * accounts @{
                 */
    ...
    public EventInfoFragment(Context context, Uri uri, long startMillis,
            ...
            int windowStyle, ArrayList<ReminderEntry> reminders) {
    ...
    public EventInfoFragment(Context context, long eventId, long startMillis,
        ...
        this(context, ContentUris.withAppendedId(Events.CONTENT_URI, eventId),
                startMillis, endMillis, attendeeResponse, isDialog,
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        final Activity activity = getActivity();
        mContext = activity;
        mColorPickerDialog = (EventColorPickerDialog) activity
                .getFragmentManager()
        ...
        mEditResponseHelper
                .setDismissListener(new DialogInterface.OnDismissListener() {
                    ...
                    public void onDismiss(DialogInterface dialog) {
                        // If the user dismisses the dialog (without hitting
                        // OK),
                        // then we want to revert the selection that opened the
                        // dialog.
                        if (mEditResponseHelper.getWhichEvents() != -1) {
                            ...
                            // Revert the attending response radio selection to
                            // whatever
                            // was selected prior to this selection (possibly
                            // nothing).
                            int oldResponse;
                            ...
                            // If the radio group is being cleared, also clear
                            // the
                            // dialog's selection of which events should be
                            // included
                            // in this response.
                            if (buttonToCheck == -1) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (savedInstanceState != null) {
            mIsDialog = savedInstanceState.getBoolean(BUNDLE_KEY_IS_DIALOG,
                    false);
            /* SPRD: Add for bug569477, tip read contact. @{ */
            mIsShowDialog = savedInstanceState.getBoolean(
                    BUNDLE_KEY_ISSHOWDIALOG, false);
            mIsSelectedFlag = savedInstanceState.getBoolean(
                    BUNDLE_KEY_ISSELECTFLAG, true);
            /* @{ */
            mWindowStyle = savedInstanceState.getInt(BUNDLE_KEY_WINDOW_STYLE,
                    DIALOG_WINDOW_STYLE);
            mDeleteDialogVisible = savedInstanceState.getBoolean(
                    BUNDLE_KEY_DELETE_DIALOG_VISIBLE, false);
            mCalendarColor = savedInstanceState
                    .getInt(BUNDLE_KEY_CALENDAR_COLOR);
            mCalendarColorInitialized = savedInstanceState
                    .getBoolean(BUNDLE_KEY_CALENDAR_COLOR_INIT);
            mOriginalColor = savedInstanceState
                    .getInt(BUNDLE_KEY_ORIGINAL_COLOR);
            mOriginalColorInitialized = savedInstanceState
                    .getBoolean(BUNDLE_KEY_ORIGINAL_COLOR_INIT);
            mCurrentColor = savedInstanceState.getInt(BUNDLE_KEY_CURRENT_COLOR);
            mCurrentColorInitialized = savedInstanceState
                    .getBoolean(BUNDLE_KEY_CURRENT_COLOR_INIT);
        ...
        mResponseRadioGroup = (RadioGroup) mView
        ...
        // SPRD Modify for bug451704/500118, move this to onResume,in order to
        // make sure photo change,not sync
        ...
        emailAttendeesButton = (Button) mView
     ...
     */
    private synchronized boolean initEventCursor() { //UNISOC: Modify for bug1227549
    ...
    // SPRD: Modify 20150226 of bug408526, change view to fianl.
    private void initAttendeesCursor(final View view) {
            ...
            if (mAttendeesCursor.moveToFirst()) {
                ...
                do {
                        ...
                        if (!TextUtils.isEmpty(name)) {
                            ...
                            if (!mIsOrganizer) {
                                setVisibilityCommon(view,
                                        R.id.organizer_container, View.GONE);      //SPRD: Modify for bug751356
                    ...
                    if (mCalendarOwnerAttendeeId == EditEventHelper.ATTENDEE_ID_NONE
                        ...
                        if (Utils.isJellybeanOrLater()) {
                            identity = mAttendeesCursor
                                    .getString(ATTENDEES_INDEX_IDENTITY);
                            idNamespace = mAttendeesCursor
                ...
                /* SPRD: Add for bug569477, tip read contact. @{ */
                mAttendeesSize = mAcceptedAttendees.size()
                        + mDeclinedAttendees.size()
                        + mTentativeAttendees.size()
                        + mNoResponseAttendees.size();
                if ("0".equals(Utils.mSecureCheck) || mAttendeesSize == 0) {
                    updateAttendees(view);
                } else {
                    if (!(mReadContactDialog != null && mReadContactDialog
                            .isShowing())) {
                        if (mIsShowDialog) {
                            if (mIsSelectedFlag) {
                                updateAttendees(view);
                            } else {
                                mIsReadContact = false;
                                updateAttendees(view);
                            }
                        } else {
                            AlertDialog.Builder builder = new AlertDialog.Builder(
                                    getActivity());
                            builder.setTitle(
                                    R.string.remind_read_userdata_title)
                                    .setMessage(R.string.read_userdata_content)
                                    .setPositiveButton(
                                            R.string.yes,
                                            new DialogInterface.OnClickListener() {
                                                ...
                                                public void onClick(
                                                        ...
                                                        int which) {
                                                    updateAttendees(view);
                                                    mIsSelectedFlag = true;
													DontDisplayContactDialog();
                                                }
                                            })
                                    .setNegativeButton(
                                            R.string.no,
                                            new DialogInterface.OnClickListener() {
                                                ...
                                                public void onClick(
                                                        DialogInterface dialog,
                                                        int which) {
														mReadContactDialog.dismiss();
                                                }
                                            });
                            mReadContactDialog = builder.create();
                            mReadContactDialog.setCanceledOnTouchOutside(false);
                            mReadContactDialog
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putBoolean(BUNDLE_KEY_CALENDAR_COLOR_INIT,
                mCalendarColorInitialized);
        outState.putInt(BUNDLE_KEY_ORIGINAL_COLOR, mOriginalColor);
        outState.putBoolean(BUNDLE_KEY_ORIGINAL_COLOR_INIT,
                mOriginalColorInitialized);
        ...
        /* SPRD: Add for bug569477, tip read contact. @{ */
        outState.putBoolean(BUNDLE_KEY_ISSHOWDIALOG, mIsShowDialog);
        outState.putBoolean(BUNDLE_KEY_ISSELECTFLAG, mIsSelectedFlag);
        /* @{ */
        // We'll need the temporary response for configuration changes.
        outState.putInt(BUNDLE_KEY_TENTATIVE_USER_RESPONSE,
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == android.R.id.home) {
            ...
            /* SPRD: Add for bug569477, tip read contact. @{ */
            if ("1".equals(Utils.mSecureCheck) && needDisplayContactDialog()) {
                doEditEvent();
            } else {
                doEdit();
                mActivity.finish();
    ...
    private void share() {
        ...
        try {
            ...
            Toast.makeText(getActivity(), R.string.no_way_to_share_calendar,
    ...
    private Uri writeToFile(Uri uri) {
        ...
        Uri osUri = FileProvider.getUriForFile(mContext,
                "com.android.calendar.fileprovider", file);
        /* @} */
    ...
    private void showEventColorPickerDialog() {
        if (mColorPickerDialog == null) {
            mColorPickerDialog = EventColorPickerDialog.newInstance(mColors,
                    mCurrentColor, mCalendarColor, mIsTabletConfig);
            mColorPickerDialog.setOnColorSelectedListener(this);
        }
    ...
    public void onStop() {
        ...
        if (!mEventDeletionStarted && act != null
            ...
            if (saveReminders() || responseSaved || eventColorSaved) {
                /*
                 * SPRD for bug455353, edit a action, if it has Attendees, toast
                 * should be "Updates will be sent". @{
                 */
                mAttendeesSize = mAcceptedAttendees.size()
                        + mDeclinedAttendees.size()
                        + mTentativeAttendees.size()
                ...
                if (mAttendeesSize > 0) {
                    Toast.makeText(getActivity(),
                            R.string.saving_event_with_guest,
                            Toast.LENGTH_SHORT).show();
                } else {
    ...
    private boolean saveResponse() {
        ...
        int status = getResponseFromButtonId(mResponseRadioGroup
    ...
    private void doEdit() {
        ...
        if (c != null) {
            ...
            intent.putExtra(EditEventActivity.EXTRA_EVENT_REMINDERS,
                    EventViewUtils.reminderItemsToReminders(mReminderViews,
                            mReminderMinuteValues, mReminderMethodValues));
        ...
        if (c != null) {
            ...
            intent.putExtra(EditEventActivity.EXTRA_EVENT_REMINDERS,
                    EventViewUtils.reminderItemsToReminders(mReminderViews,
                            mReminderMinuteValues, mReminderMethodValues));
            intent.putExtra(EVENT_EDIT_ON_LAUNCH, true);
            /* UNISOC: Modify for bug1206642 @{ */
            mReadUserDataDialog = new AlertDialog.Builder(mContext)
                    .setTitle(R.string.remind_read_userdata_title)
                    .setMessage(R.string.read_userdata_content)
                    .setPositiveButton(R.string.yes,
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog,
                                    ...
                                    intent.putExtra(
                                            CalendarController.VISIT_CONTACT,
                                            true);
                                    startActivity(intent);
                                    mActivity.finish();
                                }
                    ...
                    .setNegativeButton(R.string.no,
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog,
                                        int which) {
                                    mReadUserDataDialog.dismiss();
                                }
                            }).create();
            mReadUserDataDialog.show();
            /* @} */
        }
    }

    private void doEditEventInfo() {
        Context c = getActivity();
        // This ensures that we aren't in the process of closing and have been
        // unattached already
        if (c != null) {
            ...
            builder.setTitle(R.string.remind_read_userdata_title)
                    .setMessage(R.string.read_userdata_content)
                    .setPositiveButton(R.string.yes,
                            new DialogInterface.OnClickListener() {
    ...
    private void updateEvent(View view) {
        ...
        // 3rd parties might not have specified the start/end time when firing
        // the
            ...
            if (mEndMillis == 0) {
                ...
                if (!TextUtils.isEmpty(duration)) {
                    try {
                        ...
                        Log.d(TAG, "Error parsing duration string " + duration,
                                e);
                    }
                }
                if (mEndMillis == 0) {
        ...
        /* UNISOC: Modify for bug1227302 @{ */
        Resources resources = context.getResources();
        mEventInfoBgColor = resources.getColor(R.color.event_info_background);
        mHeadlines.setBackgroundColor(mEventInfoBgColor);
        /* @} */

        // What
        if (eventName != null) {
            setTextCommon(view, R.id.title, eventName);
            // Add for bug524609, identify information content for before and
            // after the url of the character
            SprdLinkify.addLinksSprd(mTitle, SprdLinkify.ALL);
        }
        ...
        if (!TextUtils.isEmpty(rRule)) {
            ...
            repeatString = EventRecurrenceFormatter.getRepeatString(mContext,
                    resources, eventRecurrence, true);
        }
        if (repeatString == null) {
            if(view.findViewById(R.id.when_repeat1) != null) {
                view.findViewById(R.id.when_repeat1).setVisibility(View.GONE);
            }
        } else {
        ...
        if (location == null || location.trim().length() == 0) {
            setVisibilityCommon(view, R.id.where1, View.GONE);
            setVisibilityCommon(view, R.id.line4, View.GONE);
        } else {
            ...
            if (textView != null) {
                ...
                try {
                    textView.setText(Utils.extendedLinkify(textView.getText()
                            .toString(), true));
                    // Add for bug524609, identify information content for
                    // before and after the url of the character
                    SprdLinkify.addLinksSprd(textView, SprdLinkify.ALL);
                ...
                /*
                 * SPRD: bug442319, bug491329 not map appliation installed cause
                 * crash by monkey @{
                 */
                textView.setKeyListener(new KeyListener() {
                    ...
                    public boolean onKeyDown(View view, Editable text,
                        ...
                        if (keyCode == event.KEYCODE_DPAD_CENTER
                            ...
                            // if there has no map application then not to
                            // trigger the linker
                            return !Utils
                                    .isMapApplicationAvailable(getActivity());
                        }
                        return false;
                    }
    ...
    private void updateCalendar(View view) {
        ...
        if (mCalendarsCursor != null && mEventCursor != null) {
            ...
            mHandler.startQuery(TOKEN_QUERY_VISIBLE_CALENDARS, null,
                    Calendars.CONTENT_URI, CALENDARS_PROJECTION,
            ...
            if (!mIsOrganizer && !TextUtils.isEmpty(mEventOrganizerDisplayName)) {
                ...
                setVisibilityCommon(view, R.id.organizer_container,
                        View.GONE);                      //SPRD: Modify for bug751356
            } else {
                ...
                b.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if ("1".equals(Utils.mSecureCheck) && needDisplayContactDialog()) {
                            ...
                            // For full screen, close activity on phone, leave
                            // it for tablet
                            if (mIsDialog) {
                                EventInfoFragment.this.dismiss();
                            } else if (!mIsTabletConfig) {
    ...
    private void updateAttendees(View view) {
        if (mAttendeesSize > 0) {
            ...
            /* SPRD: Add for bug569477, tip read contact. @{ */
            (mLongAttendees).addAttendees(mAcceptedAttendees, mIsReadContact);
            (mLongAttendees).addAttendees(mDeclinedAttendees, mIsReadContact);
            (mLongAttendees).addAttendees(mTentativeAttendees, mIsReadContact);
            (mLongAttendees).addAttendees(mNoResponseAttendees, mIsReadContact);
            /* @} */
            mLongAttendees.setEnabled(false);
            mLongAttendees.setVisibility(View.GONE);                 //SPRD: Modify for bug743559
        } else {
        ...
        if (hasEmailableAttendees()) {
            setVisibilityCommon(mView, R.id.email_attendees_container,
                    View.GONE);                                                      //SPRD: Modify for bug743559
            if (emailAttendeesButton != null) {
    ...
    void updateResponse(View view) {
        ...
        setVisibilityCommon(view, R.id.response_container, View.GONE);            //SPRD: Modify for bug743559
    ...
    /* UNISOC: Modify for bug1206642 @{ */
    public void dismissReadUserDataDialog() {
        if (mReadUserDataDialog != null && mReadUserDataDialog.isShowing()) {
            mReadUserDataDialog.dismiss();
            mReadUserDataDialog = null;
        }
    }
    /* @} */
    ...
    public void onPause() {
        ...
        // SPRD for bug449438/501157, we should
        // setMonthRepeatByDayOfWeekStrsNull in onPause()
    ...
    public void onResume() {
        ...
        // Display the "delete confirmation" or "edit response helper" dialog if
        // needed
        if (mDeleteDialogVisible) {
            ...
            mEditResponseHelper
                    .showDialog(mEditResponseHelper.getWhichEvents());
        }
    ...
    /*
     * SPRD: bug442745, bug491495 @{ this function copy from CalendarEventModel
    ...
    public void onColorSelected(int color) {
        ...
        mHeadlines.setBackgroundColor(mEventInfoBgColor); // UNISOC: Modify for bug1227302
    }

    private boolean needDisplayContactDialog() {
        SharedPreferences localPreferences = mActivity.getSharedPreferences("local_shareference", Activity.MODE_PRIVATE);
        Boolean firstCreate = localPreferences.getBoolean("first_create", true);
    ...
    private void DontDisplayContactDialog() {
        SharedPreferences localPreferences = mActivity.getSharedPreferences("local_shareference", Activity.MODE_PRIVATE);
        Boolean firstCreate = localPreferences.getBoolean("first_create", true);
        ...
        if(firstCreate) {
            ...
            if (editor != null) {
                editor.putBoolean("first_create", false);
                editor.commit();                
            }
        }
    }
