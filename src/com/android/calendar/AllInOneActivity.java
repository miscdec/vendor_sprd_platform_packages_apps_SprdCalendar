...
/* @} */
import com.sprd.calendar.newmonth.fragment.ScheduleFragment;
...
public class AllInOneActivity extends AbstractCalendarActivity implements
    ...
    /*
     * SPRD: for bug 441656 , change the start day of week, the time can't be
     * located to the current time @{
     */
    ...
    // SPRD: Add for bug596482, Read user data box will disappear when
    // orientation the screen
    ...
    protected void onCreate(Bundle icicle) {
        ...
        /* SPRD modify for bug659284 {@ */
        if (Utils.isIntentFromMalware(getIntent())) {
            finish();
            return;
        }
        /* @} */
         ...
         */
        mPrefs = CalendarUtils.getSharedPreferences(this,
        ...
        if (icicle != null && icicle.containsKey(BUNDLE_KEY_SEARCHVIEW)) {
            mIsSearchViewVisibility = icicle.getBoolean(BUNDLE_KEY_SEARCHVIEW,
                    false);
        }
        /* @} */
        /* SPRD: Add for bug535085, delete all events @{ */
        if (icicle != null && icicle.containsKey(BUNDLE_KEY_DELETEALL)) {
            mShowDeleteAllDialog = icicle.getBoolean(BUNDLE_KEY_DELETEALL,
                    false);
        }
        if (mShowDeleteAllDialog) {
            showDeleteAllDialog();
        }
        /* @} */
        ...
        timeMillis = Utils.limitDate(mTimeZone, timeMillis);
        /* @} */
        ...
        mFabButton.setOnClickListener(new OnClickListener() {
            public void onClick(View arg0) {
                ...
                mController.sendEventRelatedEvent(this, EventType.CREATE_EVENT,
        ...
        /*
         * SPRD: bug474714 20150921 go-to-date dialog can not successfully
         * locate when switch scr een orientation @{
         */
         ...
         * orientation the screen @}
         */
        if (icicle != null && icicle.containsKey(BUNDLE_KEY_READCONTACT)) {
            mShowReadContactDialog = icicle.getBoolean(BUNDLE_KEY_READCONTACT,
                    false);
        }
        if (mShowReadContactDialog) {
            Time t1 = null;
            t1 = new Time();
            t1.set(mController.getTime());
            if (t1.minute > 30) {
                t1.hour++;
                t1.minute = 0;
            } else if (t1.minute > 0 && t1.minute < 30) {
                t1.minute = 30;
            }
            mController.sendEventRelatedEvent(this, EventType.CREATE_EVENT, -1,
                    t1.toMillis(true), 0, 0, 0, -1);
        }
        /* @} */
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        // SPRD: Modify for bug393301/520630. mSearchView is a null object
        // reference;
        // mIsSearchViewVisibility = mSearchView.isShown();
        mSearchContent = mSearchView == null ? "" : mSearchView.getQuery();
        outState.putCharSequence(BUNDLE_KEY_SEARCH_CONTENT, mSearchContent);
        if (mSearchView != null) {
            mIsSearchViewVisibility = mSearchView.isShown();
        ...
        /* @} */
        /* SPRD: Add for bug535085, delete all events @{ */
        if (null != mDelAllAlertDialog) {
            mShowDeleteAllDialog = mDelAllAlertDialog.isShowing();
        }
        outState.putBoolean(BUNDLE_KEY_DELETEALL, mShowDeleteAllDialog);
        /* @} */

        // SPRD: Add for bug596482, Read user data box will disappear when
        // orientation the screen
        mShowReadContactDialog = mController.isReadContactDialogShowing();
        outState.putBoolean(BUNDLE_KEY_READCONTACT, mShowReadContactDialog);
    ...
    protected void onDestroy() {
        ...
        if (mController != null) {
            ...
            // SPRD: Add for bug645179, Read user data box will disappear when
            // orientation the screen
            mController.dismissContactDialog();
        }
        /* @} */
    }
    ...
    private void initFragments(long timeMillis, int viewType, Bundle icicle) {
        ...
        if (viewType == ViewType.EDIT) {
            ...
            // SPRD 559624
            // don't set current view type to preview type,it will make back key
            // useless
    ...
    public void onBackPressed() {
        if (mCurrentView == ViewType.EDIT || mBackToPreviousView) {
            mController.sendEvent(this, EventType.GO_TO, null, null, -1,
                    mPreviousView);
        /* SPRD: for Bug666952, Calendar hot start performance issue @{ */
        /* SPRD: for Bug743958, Calendar exit when click back key in search mode @{ */
        } else if(mSearchView != null && mSearchView.isShown()){
            super.onBackPressed();
        } else {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (mSearchView != null) {
            ...
            int search_close_btn = mSearchView.getContext().getResources().getIdentifier("android:id/search_close_btn", null, null);
            int search_go_btn = mSearchView.getContext().getResources().getIdentifier("android:id/search_go_btn", null, null);
            TextView textView = (TextView) mSearchView.findViewById(searchSrcTextId);
            ImageView imageView = (ImageView) mSearchView.findViewById(search_close_btn);
            ImageView go_iv = (ImageView) mSearchView.findViewById(search_go_btn);
            imageView.setImageResource(R.drawable.ic_jing_cancel);
            go_iv.setImageResource(R.drawable.ic_jing_done);
            mSearchView.setSubmitButtonEnabled(true);
            //Todo actionbar上搜索输入框的hint字体颜色设置
            textView.setHintTextColor(getResources().getColor(R.color.agenda_event_past_text));
            textView.setTextColor(getResources().getColor(R.color.normal_text_color));
            SprdTextWatcher editTextWatcher = new SprdTextWatcher(this, textView, MAX_SEARCH_LENGTH);
        ...
        mTodayMenu = menu.findItem(R.id.action_today);
        /* UNISOC: Modify for bug 1030121 @{ */
        long time = Utils.limitDate(mTimeZone,mController.getTime());
        if (mController.getViewType() == ViewType.MONTH && isToday(time)) {
        ...
        if (Utils.isJellybeanOrLater()) {
            ...
            // LayerDrawable icon = (LayerDrawable) mTodayMenu.getIcon(); // UNISOC: Modify for bug1211058
            // Utils.setTodayIcon(icon, this, mTimeZone);
            /* SPRD: Add for bug535085, delete all events @{ */
            int viewType = mController.getViewType();
            mDeleteAllMenu = menu.findItem(R.id.action_delete_all_events);
            /* UNISOC: Modify for bug1224502 @{ */
            if (mDeleteAllMenu != null) {
                if (viewType == ViewType.AGENDA) {
                    /* SPRD: for Bug249018, set the clear menu status @{ */
                    if (mDeleteAllMenu.isVisible()) {
                        DeleteEventHelper helper = new DeleteEventHelper(this,
                                this, false);
                        mDeleteAllMenu.setEnabled(helper.hasEvents());
                    }
                    /* @} */
                    /* SPRD: for Bug739751, set the clear menu visible @{ */
                    mDeleteAllMenu.setVisible(true);
                    /* @} */
                } else {
                    mDeleteAllMenu.setVisible(false);
                }
            }
            /* @} */
           /*  @} */
            ...
            mTodayMenu.setIcon(R.drawable.ic_menu_today_no_date_holo_light);
        }
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.action_refresh) {
            ...
            return true;
            /* SPRD: Add for bug535085, delete all events @{ */
        } else if (itemId == R.id.action_delete_all_events) {
            showDeleteAllDialog();
            return true;
            /* @} */
        ...
        /* UNISOC: Modify for bug 1030121 @{ */
        long tMillis = Utils.limitDate(mTimeZone, t.toMillis(true));
        t.set(tMillis);
    ...
    public void setControlsOffset(int controlsOffset) {
        if (mOrientation == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            mControlsParams.width = Math.max(0, mControlsAnimateWidth
                    - controlsOffset);
            ...
            mVerticalControlsParams.height = Math.max(0, mControlsAnimateHeight
                    - controlsOffset);
    ...
    /* UNISOC: Modify for bug1237903 @{ */
    private void setSelected(ActionBar.Tab currentTab, int currentButton) {
        if (mActionBar != null
                && (mActionBar.getSelectedTab() != currentTab)) {
            mActionBar.selectTab(currentTab);
        }
        if (mActionBarMenuSpinnerAdapter != null && mActionBar != null) {
            mActionBar
                    .setSelectedNavigationItem(currentButton);
        }
        if (mTodayMenu != null) {
    ...
    private void setMainPane(FragmentTransaction ft, int viewId, int viewType,
        ...
        /* @} */
        /* UNISOC: Modify for bug1237903 @{ */
        if (mOnSaveInstanceStateCalled || (!force && mCurrentView == viewType)) {
            return;
        }
        /* @} */
        ...
        if (mCurrentView == ViewType.AGENDA && (oldFrag instanceof AgendaFragment)) {
        ...
        // SPRD: for bug 441656 , change the start day of week, the time can't
        // be located to the current time
        boolean startweekchange = mPrefs.getBoolean(
                Utils.WEEK_START_DAY_CHANGE, false);
        switch (viewType) {
        case ViewType.AGENDA:
            /* UNISOC: Modify for bug1237903 @{ */
            setSelected(mAgendaTab, CalendarViewAdapter.AGENDA_BUTTON_INDEX);
            /* @} */
            /*
            ...
            ExtensionsFactory.getAnalyticsLogger(getBaseContext()).trackView(
                    "agenda");
            /* SPRD: for Bug535085/249018, set the clear menu status @{ */
            if (mDeleteAllMenu != null && mDeleteAllMenu.isVisible()) {
                DeleteEventHelper helper = new DeleteEventHelper(this, this,
                        false);
                mDeleteAllMenu.setEnabled(helper.hasEvents());
            }
            /* @} */
        ...
        case ViewType.DAY:
            /* UNISOC: Modify for bug1237903 @{ */
            setSelected(mDayTab, CalendarViewAdapter.DAY_BUTTON_INDEX);
            /* @} */
            /*
             * SPRD: for bug 441656 , change the start day of week, the time
             * can't be located to the current time @{
            ...
            if (mDayFrag != null && startweekchange == true) {
                ...
                frag = DayFragment.newInstance(timeMillis, 1); // UNISOC: Modify for bug1145293
                mDayFrag = frag;
            }
            /* @} */
        ...
        case ViewType.MONTH:
            /* UNISOC: Modify for bug1237903 @{ */
            setSelected(mMonthTab, CalendarViewAdapter.MONTH_BUTTON_INDEX);
            /* @} */
            /*
             * SPRD: for bug 441656 , change the start day of week, the time
             * can't be located to the current time @{
            ...
            if (mMonthFrag != null && startweekchange == true) {
                ...
                if (Utils.mUseOldMonthView) {
                    frag = new MonthByWeekFragment(timeMillis, false);
                } else {
                    frag = ScheduleFragment.newInstance(timeMillis); // SPRD: Add for bug732533,1145293 Add init date with select date
                }
                mMonthFrag = frag;
            }
            /* @} */
        ...
        default:
            /* UNISOC: Modify for bug1237903 @{ */
            setSelected(mWeekTab, CalendarViewAdapter.WEEK_BUTTON_INDEX);
            /* @} */
            /*
             * SPRD: for bug 441656 , change the start day of week, the time
             * can't be located to the current time @{
            ...
            if (mWeekFrag != null && startweekchange == true) {
                ...
                frag = DayFragment.newInstance(timeMillis, 7); // UNISOC: Modify for bug1145293
                mWeekFrag = frag;
            }
            /* @} */
            mFabButton.setVisibility(View.VISIBLE);
    ...
    /* UNISOC: Modify for bug474714,1232836, Add "select date" feature @{ */
    private void showDateDialog(Time time) {
        ...
        datePicker.setFirstDayOfWeek(Utils.getFirstDayOfWeekAsCalendar(this));
        datePicker.setMinDate(0); // 1970-01-01
        datePicker.setMaxDate(Utils.END_DAY_MAX - 1); // 2037-12-31
        /*
...
//        }
    ...
    private class DateListener implements OnDateSetListener {
        @Override
        public void onDateSet(DatePicker view, int year, int month, // UNISOC: Modify for bug1232836
                int monthDay) {
            ...
            time.normalize(true);
            mController.sendEvent(this, EventType.GO_TO, time, null, time, -1,
                    mCurrentView, CalendarController.EXTRA_GOTO_DATE, null,
                    null);
        }
    }
    ...
    private void showDeleteAllDialog() {
        mDelAllAlertDialog = new AlertDialog.Builder(this)
                ...
                .setPositiveButton(android.R.string.ok,
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog,
                                    int button) {
                                execEventsDel(AllInOneActivity.this);
                            }
                        }).setNegativeButton(android.R.string.cancel, null)
                .create();
        mDelAllAlertDialog.show();
    }

    /* @} */

    /* SPRD: Add for bug535085, delete all events @{ */
    private void execEventsDel(Activity activity) {
        final DeleteEventHelper mDeleteEventHelper = new DeleteEventHelper(
                this, this, false);
        mDeleteEventHelper.deleteAllEvents();
        /* SPRD: for Bug249018, set the clear menu status @{ */
        if (mDeleteAllMenu != null && mDeleteAllMenu.isVisible()) {
            mDeleteAllMenu.setEnabled(false);
        }
        /* @} */
    }
    ...
    public void setDateForActionButton(long time) {
        if (mActionBarMenuSpinnerAdapter != null) {
            mActionBarMenuSpinnerAdapter.setTime(time);
        }
    }

    public void setTodayMenuShow(boolean show) {
        if (mTodayMenu != null) {
            mTodayMenu.setVisible(show);
        }
    }

   //use to check selecttime is not today for show today button
   public boolean isToday(long time){
        Time nowTime = new Time();
        nowTime.setToNow();
        Time backTime = new Time();
        backTime.set(time);
        return backTime.year == nowTime.year && backTime.month == nowTime.month && backTime.monthDay == nowTime.monthDay;
   }
}
