...
public class AgendaByDayAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Resources res = mContext.getResources(); // UNISOC: Modify for bug1232836
        ...
        if (row.mType == TYPE_DAY) {
            ...
            if (row.mDay > mTodayJulianDay) {
                ...
                agendaDayView.setBackgroundColor(res.getColor(R.color.day_past_background_color));
                holder.grayed = true;
            }
            ...
            if ((!allDay && row.mEventStartTimeMilli <= System
                ...
                /* UNISOC: Modify for bug1232836 @{ */
                itemView.setBackgroundColor(res.getColor(R.color.day_past_background_color));
                title.setTypeface(Typeface.DEFAULT);
                title.setTextColor(res.getColor(R.color.agenda_event_past_text));
                when.setTextColor(res.getColor(R.color.agenda_event_past_text));
                where.setTextColor(res.getColor(R.color.agenda_event_past_text));
                /* @} */
                holder.grayed = true;
            } else {
    ...
    public void calculateDays(DayAdapterInfo dayAdapterInfo) {
        ...
        for (int position = 0; cursor.moveToNext(); position++) {
            ...
            if (endDay > startDay) {
                long nextMidnight = Utils.getNextMidnight(tempTime, startTime,
                        mTimeZone);
                multipleDayList.add(new MultipleDayInfo(position, endDay, id,
                        nextMidnight, endTime, instanceId, allDay));
                // Add in the event for this cursor position - since it is the
                // start of a multi-day
                // event, the end time is midnight
                rowInfo.add(new RowInfo(TYPE_MEETING, startDay, position, id,
                        startTime, nextMidnight, instanceId, allDay));
            } else {
