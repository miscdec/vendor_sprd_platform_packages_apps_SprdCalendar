...
public class MonthByWeekAdapter extends SimpleWeeksAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        /* SPRD: Add for bug474830 set day high light. @{ */
        Log.d(TAG, "View getView isAnimatingSelectedDay = "
                + isAnimatingSelectedDay);
        if (isAnimatingSelectedDay) {
            drawingParams.put(
                    MonthWeekEventsView.VIEW_PARAMS_ANIMATE_SELECTED_DAY, 1);
            mAnimateSelectedDay = false;
        }
    ...
    public void setRealSelectedWeek() {
        ...
        mRealSelectedWeek = Utils.getWeeksSinceEpochFromJulianDay(
                Time.getJulianDay(millis, mRealSelectedDay.gmtoff),
                mFirstDayOfWeek);
    }

    private boolean isHasSelectedDay(Time t, int expectWeek) {
        int julianDay = Time.getJulianDay(t.toMillis(false), t.gmtoff);
        int week = Time.getWeeksSinceEpochFromJulianDay(julianDay,
                Utils.getFirstDayOfWeek(mContext));
