...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    /* SPRD: Add for bug 474830, set day high light. @{ */
    ...
    private final SelectedDayAnimatorListener mSelectedAnimatorListener = new SelectedDayAnimatorListener();
    /* @} */
    ...
    protected void loadColors(Context context) {
        ...
        /* SPRD: Add for bug 474830, set day high light. @{ */
        mMonthBGSetdayColor = res.getColor(R.color.month_setday_bgcolor);
        /* @} */
    }
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        if (params.containsKey(VIEW_PARAMS_ANIMATE_SELECTED_DAY) && mHasSelectedDay) {
            synchronized (mSelectedAnimatorListener) {
                ...
                mSelectedAnimatorListener.setAnimator(mSelectedDayAnimator);
                mSelectedAnimatorListener.setFadingIn(true);
                mSelectedDayAnimator.addListener(mSelectedAnimatorListener);
                mAnimateSelectedDay = true;
                Log.d(TAG, "setWeekParams VIEW_PARAMS_ANIMATE_SELECTED_DAY mSelectedDayAnimator= " + mSelectedDayAnimator);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        /* SPRD: Add for bug474830, set day high light. @{ */
        Log.d(TAG, "onDraw enter mHasSelectedDay = " + mHasSelectedDay + " mAnimateSelectedDay" + mAnimateSelectedDay);
        ...
        for (; i < numCount; i++) {
            ...
            if (Utils.mLunarFlag || Utils.mSupportForeignFestivalCalendar) {
                ...
                if (lunarStr.contains("/")) {
                    ...
                    if (mOrientation == Configuration.ORIENTATION_LANDSCAPE) {
                        /* SPRD: Add for bug467636, add foreign festival info. @{ */
                        if(Utils.mSupportForeignFestivalCalendar && lunarStr.contains("-/")){
                            lunarStr=lunarStr.replace("-/","");
                        }
                        canvas.drawText(lunarStr, lunarX, lunarY,
                                mMonthNumPaint);
                    } else {
                        for(j = 0; j < count; j++){
                            //SPRD: Add for bug543130, word overlaps on whale2
                            canvas.drawText(strArray.get(j), lunarX, lunarY + j*(MINI_DAY_NUMBER_TEXT_SIZE + 6),
                                    mMonthNumPaint);
                        }
                    }
                } else {
    ...
    protected void drawSelectedDay(Canvas canvas) {
        ...
        r.right = computeDayLeftPosition(mSelectedDayIndex + 1)
                - (int) Math.ceil(TODAY_HIGHLIGHT_WIDTH / 2.0f);
        p.setColor(mMonthBGSetdayColor);
        Log.d(TAG, "drawSelectedDay mSelectedDayIndex = " + mSelectedDayIndex + " left = " + r.left + " right = " + r.right + " r.top = " + r.top + " r.bottom = " + r.bottom);
        canvas.drawRect(r, p);
        p.setStyle(Style.FILL);
    }
    private int updateSelectedDayIndex(int weekDay) {
        ...
        Log.d(TAG, "updateSelectedDayIndex mSelectedDayIndex = " + mSelectedDayIndex);
