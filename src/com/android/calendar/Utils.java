...
// SPRD: for bug 441656 , change the start day of week, the time can't be located to the current time
    ...
    public static final boolean mSupportForeignFestivalCalendar = false;
    //if (SystemProperties.get("persist.support.foreigncalendar").equals("true")){
    //    mSupportForeignFestivalCalendar = true;
    //}
    /* @} */

    public static final boolean mUseOldMonthView = false;

    //SPRD: Add for bug569477, tip read contact.
    public static String mSecureCheck = SystemProperties.get("persist.support.securetest" ,"1");
    /* UNISOC: Modify for bug1188866  {@ */
    public static String SERVICE_NOTIFICATION_CHANNEL = "SERVICE_NOTIFICATION_CHANNEL";
    public static String SERVICE_NOTIFICATION_GROUP = "SERVICE_NOTIFICATION_GROUP";
    /* }@ */
    /**
    ...
    **/
    ...
    public static boolean getDefaultVibrate(Context context, SharedPreferences prefs) {
        ...
        if (prefs.contains(KEY_ALERTS_VIBRATE_WHEN)) {
            ...
            prefs.edit().remove(KEY_ALERTS_VIBRATE_WHEN).apply();
    ...
    /*SPRD:Add for bug :729879 @{*/
    /**
     *It's used to show weeks.
     *@Returns the current first day of week and  the difference to be set.
    */
    public static int getFirstDayOfWeekDiff(Context context) {
        Time time = new Time();
        int mWeekStart = getFirstDayOfWeek(context);
        return time.weekDay - mWeekStart;
    }
    /*}@*/
    ...
    public static void setMidnightUpdater(Handler h, Runnable r, String timezone) {
        ...
        time.set(now);
        /* UNISOC: Modify for bug1214981  {@ */
        long Millis = 24 * 3600 - time.hour * 3600 - time.minute * 60 - time.second + 1;
        long runInMillis = Millis * 1000;
        /* }@ */
        h.removeCallbacks(r);
    ...
    public static String getDisplayedDatetime(long startMillis, long endMillis, long currentMillis,
        ...
        if (allDay) {
            ...
            /* SPRD: for bug517909,1071752 New an allDay event of 1970.1.1，view the event from DayView, display error @{ */
    ...
    public static long limitDate(String timeZone, long oldMillis) {
        long newMillis;
        Time t = new Time(timeZone);
        t.setToNow();
        int goToJulianDay = Time.getJulianDay(oldMillis, t.gmtoff);
        ...
        } else if (goToJulianDay > MAX_DATE_JULIAN_DAY) {
            goToJulianDay = MAX_DATE_JULIAN_DAY;
            t.setJulianDay(goToJulianDay);
            newMillis = t.normalize(true);
        } else {
            newMillis = oldMillis;
    ...
    public static int[] limitMaxDate(int year, int month,int day) {
        int date[] = new int[3];
        if (year > YEAR_MAX) {
            date[0] = YEAR_MAX;
            date[1] = MONTH_MAX;
    ...
    public static Calendar limitMaxCalendar(Calendar oldCalendar) {
        Calendar newCalendar = Calendar.getInstance();
        int date[] = limitMaxDate(oldCalendar.get(Calendar.YEAR),
                oldCalendar.get(Calendar.MONTH),
                oldCalendar.get(Calendar.DAY_OF_MONTH));
        newCalendar.set(date[0],date[1],date[2]);
        return newCalendar;
    }
    /* @} */
    ...
    public static boolean hasDeleted(Context context, Uri uri) {
        /* SPRD for bug596209 judge the song whether deleted @} */
    ...
    // SPRD: add for bug596209 get ringtone address
    public static String getRingtoneAddressFromUri(final Context context, Uri uri) {
        String stringAddress = null;

        Uri currentUri = uri;
        Uri notification_sound = Uri.parse("content://settings/system/notification_sound");
        if (currentUri.equals(notification_sound)) {
            /* SPRD: Modify for bug683471 @{ */
            String notifiUriString = Settings.System.getString(context.getContentResolver(),
                        Settings.System.NOTIFICATION_SOUND);
            if (!TextUtils.isEmpty(notifiUriString)) {
               currentUri = Uri.parse(notifiUriString);
            }
            /* @} */
        }
        final String scheme = uri.getScheme();
        if (scheme != null) { // UNISOC: Modify for bug1214981
            if (scheme.equals(ContentResolver.SCHEME_CONTENT)) {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    /** Fix bug 628429 Thirdparty application Uri is wrong caused calendar crash {@ **/
                    try {
                        stringAddress = cursor.getString(columnIndex);
                    } catch (IllegalStateException ex) {
                        Log.e(TAG, "GET RINGTONE ADDRESS FROM URI", ex);
                        ex.printStackTrace();
                    }
                    /** @} **/
                }
                /* UNISOC: Modify for bug1179181 @{ */
                if (cursor != null) {
                    cursor.close();
                }
                /* @} */
    ...
    /* SPRD modify for bug659284  {@ */
    public static boolean isIntentFromMalware(Intent intent) {
        boolean isFromMalware = false;
        try {
            intent.getIntExtra(TEST_INTENT_KEY, -1);
        } catch (RuntimeException e) {
            Log.e("IntentUtilities", "intent probly from malware intent is:" + intent, e);
            isFromMalware = true;
        }
    ...
    public static void setStatusBarColor(Activity activity) {
        ...
        int statusBarColor = activity.getResources().getColor(R.color.primary_color_dark);
    ...
    /* SPRD modify for bug732555  add max and min date  {@ */
    public static int getMonthDefaultPosition(){
        Calendar calendar = Calendar.getInstance();
        return (((calendar.get(Calendar.YEAR) - YEAR_MIN) * 12) + (calendar.get(Calendar.MONTH)));
    }

    public static int getWeekDefaultPosition() {
        Time startTime = new Time();
        Time nowTime = new Time();
        startTime.year = YEAR_MIN;
        startTime.month = 0;
        startTime.monthDay = 1;
        startTime.normalize(true);
        nowTime.set(System.currentTimeMillis());
        nowTime.normalize(true);
        return Math.abs(((Time.getJulianDay(
                    startTime.toMillis(true), startTime.gmtoff)
                    - Time.getJulianDay(System.currentTimeMillis(),
                            nowTime.gmtoff)) + 4) /7) + 1;                        //SPRD modify for bug748960 can't move to last week
    }
    /* @} */
}
