...
public class CalendarViewAdapter extends BaseAdapter {
    ...
    public View getDropDownView(int position, View convertView, ViewGroup parent) {
        View v = mInflater.inflate(R.layout.actionbar_pulldown_menu_button,
                parent, false);
    ...
    private String buildDayOfWeek() {
        ...
        /*
         * SPRD: Modify for bug473571, add lunar info @{ long julianDay =
         * Time.getJulianDay(mMilliTime,t.gmtoff); String dayOfWeek = null;
         * mStringBuilder.setLength(0);
         * 
         * if (julianDay == mTodayJulianDay) { dayOfWeek =
         * mContext.getString(R.string.agenda_today,
         * DateUtils.formatDateRange(mContext, mFormatter, mMilliTime,
         * mMilliTime, DateUtils.FORMAT_SHOW_WEEKDAY, mTimeZone).toString()); }
         * else if (julianDay == mTodayJulianDay - 1) { dayOfWeek =
         ...
         * DateUtils.formatDateRange(mContext, mFormatter, mMilliTime,
         * mMilliTime, DateUtils.FORMAT_SHOW_WEEKDAY, mTimeZone).toString()); }
         * else { dayOfWeek = DateUtils.formatDateRange(mContext, mFormatter,
         * mMilliTime, mMilliTime, DateUtils.FORMAT_SHOW_WEEKDAY,
         * mTimeZone).toString(); } return dayOfWeek.toUpperCase();
         */

        /* SPRD: Add for bug467636, add foreign festival info. @{ */
        if (Utils.mLunarFlag && !Utils.mSupportForeignFestivalCalendar) {
            LunarCalendar lunarInfo = new LunarCalendar(mContext);
            LunarCalendarConvertUtil.parseLunarCalendar(t.year, t.month,
                    t.monthDay, lunarInfo);
            ...
            String dayOfWeek = DateUtils.formatDateRange(mContext, mFormatter,
                    mMilliTime, mMilliTime, DateUtils.FORMAT_SHOW_WEEKDAY,
                    mTimeZone).toString();
            ...
            if (julianDay == mTodayJulianDay) {
                dayOfWeek = mContext.getString(
                        ...
                        DateUtils.formatDateRange(mContext, mFormatter,
                                mMilliTime, mMilliTime,
                                DateUtils.FORMAT_SHOW_WEEKDAY, mTimeZone)
                                .toString());
    ...
    private String buildMonthOfWeek() {
        mStringBuilder.setLength(0);
        String dayOfWeek = DateUtils.formatDateRange(mContext, mFormatter,
                mMilliTime, mMilliTime, DateUtils.FORMAT_SHOW_WEEKDAY,
                Time.getCurrentTimezone()).toString(); // UNISOC: Modify for bug1169506
        return dayOfWeek;
    }
    ...
    private String buildFullDate() {
        mStringBuilder.setLength(0);
        String date = DateUtils.formatDateRange(mContext, mFormatter,
                mMilliTime, mMilliTime,
    ...
    private String buildMonthYearDate() {
        ...
        String date = DateUtils.formatDateRange(
                ...
                DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_NO_MONTH_DAY
                        | DateUtils.FORMAT_SHOW_YEAR, Time.getCurrentTimezone()).toString(); // UNISOC: Modify for bug1169506
        return date;
    }
    ...
    private String buildMonthDate() {
        mStringBuilder.setLength(0);
        /* SPRD:Modified for bug 869023 @{*/
        String date;
        if (!Locale.getDefault().getLanguage().toLowerCase().contains("zh")) {
            date = DateUtils.formatDateRange(
                mContext,
                mFormatter,
                mMilliTime,
                mMilliTime,
                DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_NO_YEAR
                        | DateUtils.FORMAT_NO_MONTH_DAY, Time.getCurrentTimezone()).toString(); // UNISOC: Modify for bug1169506
        } else {
            date = DateUtils.formatDateRange(
                mContext,
                mFormatter,
                mMilliTime,
                mMilliTime,
                DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_NO_YEAR, mTimeZone).toString().substring(0,2);
            if (!date.contains(ZH_MONTH_SUFFIX)) {
                date = date+ZH_MONTH_SUFFIX;
            }
        }
        /* }@ */
        return date;
    ...
    private String buildWeekDate() {
        ...
        mStringBuilder.setLength(0);
        String date = DateUtils.formatDateRange(mContext, mFormatter,
                weekStartTime, weekEndTime, flags, mTimeZone).toString();
        return date;
    }

    private String buildWeekNum() {
        int week = Utils.getWeekNumberFromTime(mMilliTime, mContext);
        return mContext.getResources().getQuantityString(R.plurals.weekN, week,
                week);
    }
    ...
    public String updateLunaryear() {
        ...
        LunarCalendarConvertUtil.parseLunarCalendarYear(time.year, time.month,
                time.monthDay, lunarInfo);
