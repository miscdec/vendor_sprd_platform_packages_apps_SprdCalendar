...
public class AlertReceiver extends BroadcastReceiver {
    ...
    /* UNISOC: Modify for bug1202613 @{ */
    public static final String NEW_ALERTS_CHANNEL = "NEW_ALERTS_NOTIFICATION_CHANNEL";
    public static final String OLD_ALERTS_CHANNEL = "OLD_ALERTS_NOTIFICATION_CHANNEL";
    private static String mAlertChannelId;
    /* @} */
    ...
    public static void createAlertsNotificationChannel(Context context, String channelId) {
        ...
        if (NEW_ALERTS_CHANNEL.equals(channelId)) {
            notifiUriString = Settings.System.DEFAULT_NOTIFICATION_URI.toString();
            channelName = context.getString(R.string.preferences_reminder_title);
        } else {
            notifiUriString = null;
            channelName = context.getString(R.string.preferences_alerts_title);
        }

        setAlertsChannelId(channelId);
        NotificationChannel alartsChannel = new NotificationChannel(mAlertChannelId,
                channelName, NotificationManager.IMPORTANCE_DEFAULT);
        alartsChannel.enableLights(true);
        alartsChannel.setLightColor(Utils.LED_ARGB);
        alartsChannel.setSound(TextUtils.isEmpty(notifiUriString ) ? null
                : Uri.parse(notifiUriString ),Notification.AUDIO_ATTRIBUTES_DEFAULT);
        notificationMgr.createNotificationChannel(alartsChannel);
    }
    /* @} */

    /* UNISOC: Modify for bug1202613 @{ */
    public static void setAlertsChannelId(String channelId) {
        mAlertChannelId = channelId;
    }
    /* @} */
    ...
    private static Notification buildBasicNotification(Notification.Builder notificationBuilder,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            notificationBuilder.setChannelId(mAlertChannelId); // UNISOC: Modify for bug1202613
        }
    ...
    public static NotificationWrapper makeExpandingNotification(Context context, String title,
        ...
        if (Utils.isJellybeanOrLater()) {
            ...
            basicBuilder.setCategory(Notification.CATEGORY_EVENT);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                basicBuilder.setChannelId(mAlertChannelId); // UNISOC: Modify for bug1202613
            }
