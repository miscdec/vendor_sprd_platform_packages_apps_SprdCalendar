...
public class AlertService extends Service {
    ...
    void processMessage(Message msg) {
        ...
        if (providerReminder ||
                ...
                action.equals(Intent.ACTION_TIME_CHANGED) || // UNISOC: Modify for bug188833
            ...
            if (action.equals(Intent.ACTION_PROVIDER_CHANGED)) {
                try {
                    // SPRD: bug 653990 New event to remind the event of no sound
                    Thread.sleep(8000);
                } catch (Exception e) {
                    // Ignore.
                }
    ...
    static boolean updateAlertNotification(Context context) {
        ...
        /* UNISOC: Modify for bug1142537,1202613 @{ */
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            AlertReceiver.createAlertsNotificationChannel(context, AlertReceiver.NEW_ALERTS_CHANNEL);
        }
        /* @} */
        ...
        /* SPRD: Modify for bug396165,502497,1206507, Cancel event reminders, dialog does not disappear. @{ */
    ...
    static int processQuery(final Cursor alertCursor, final Context context,
        ...
        try {
            while (alertCursor.moveToNext()) {
                ...
                if (values.size() > 0) {
                    /* SPRD 743031 Calendar crashed when internal storage is full @{ */
                    try{
    ...
    private static void postNotification(NotificationInfo info, String summaryText,
        ...
        if (info.newAlert) {
            ...
            AlertReceiver.setAlertsChannelId(AlertReceiver.NEW_ALERTS_CHANNEL);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            AlertReceiver.createAlertsNotificationChannel(context, AlertReceiver.OLD_ALERTS_CHANNEL);
        }
        /* @} */
        ...
        /* SPRD: add for bug723230,1150809 not light led when close NOTIFICATION_LIGHT_PULSE @{ */
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            try {
                needlight = Settings.System.getInt(context.getContentResolver(),
            ...
            addNotificationOptions(notification, quietUpdate, tickerText,
                    ...
                    needlight); /* Show the LED for these non-expired events */
        } else if (!quietUpdate && !TextUtils.isEmpty(tickerText)) {
            notification.mNotification.tickerText = tickerText;
        }
        /* @{ */

        /* UNISOC: Modify for bug534998,1150809 @{ */
        /*try {
            if ((Settings.Global.getInt(context.getContentResolver(), Settings.Global.ZEN_MODE)) == 0) {
                PowerManager.WakeLock wakeLock;
                PowerManager pm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
                wakeLock = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP |
                        PowerManager.ON_AFTER_RELEASE, "alertService");
