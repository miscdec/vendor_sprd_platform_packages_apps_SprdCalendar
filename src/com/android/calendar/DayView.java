...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    private Context mContext; // UNISOC: Modify for bug1218596
        ...
        public void run() {
            if (mClickedEvent != null) {
                mController.sendEventRelatedEvent(this, EventType.VIEW_EVENT,
                        ...
                        mClickedEvent.endMillis, DayView.this.getWidth() / 2,
        ...
        public void onAnimationEnd(Animator animation) {
            synchronized (this) {
                ...
                if (mFadingIn) {
                    ...
                    mTodayAnimator = ObjectAnimator.ofInt(DayView.this,
    ...
    private void switchViews(boolean trackBallSelection) {
        ...
        if (mNumDays > 1) {
            ...
            if (trackBallSelection) {
                if (selectedEvent == null) {
                    ...
                    mController.sendEventRelatedEvent(this,
                            EventType.VIEW_EVENT, selectedEvent.id,
                            selectedEvent.startMillis, selectedEvent.endMillis,
                            0, 0, getSelectedTimeInMillis());
                }
            } else {
            ...
            if (selectedEvent == null) {
                ...
                mController.sendEventRelatedEventWithExtra(this,
                        EventType.CREATE_EVENT, -1, startMillis, endMillis, -1,
                        -1, extraLong, -1);
            } else {
                if (mIsAccessibilityEnabled) {
                    mAccessibilityMgr.interrupt();
                ...
                mController.sendEventRelatedEvent(this, EventType.VIEW_EVENT,
                        selectedEvent.id, selectedEvent.startMillis,
                        selectedEvent.endMillis, 0, 0,
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if ((selectionDay < mFirstJulianDay) || (selectionDay > mLastJulianDay)) {
            ...
            mController.sendEvent(this, EventType.GO_TO, date, end, -1,
                    ViewType.CURRENT);
            return true;
        }
        if (mSelectionDay != selectionDay) {
            ...
            mController.sendEvent(this, EventType.GO_TO, date, date, -1,
                    ViewType.CURRENT);
        }
    ...
    private void sendAccessibilityEventAsNeeded(boolean speakEvents) {
        if (mAccessibilityMgr == null || !mAccessibilityMgr.isEnabled()) {// Fix
                                                                          // bug
                                                                          // 622161,
    ...
    private void drawAmPm(Canvas canvas, Paint p) {
        ...
        if (mFirstHour < 12 && mFirstHour + mNumHours > 12) {
            // Also draw the "PM"
            text = mPmString;
            y = mFirstCell + mFirstHourOffset + (12 - mFirstHour)
                    * (mCellHeight + HOUR_GAP) + 2 * mHoursTextHeight
                    + HOUR_GAP;
            // SPRD: Add for bug382427/521691, dynamically calculates the
            // distance.
            // canvas.drawText(text, HOURS_LEFT_MARGIN, y, p);
            canvas.drawText(text, mAmPmLeftMargin, y, p);
        }
    ...
    private void drawCurrentTimeLine(Rect r, final int day, final int top,
            Canvas canvas, Paint p) {
        r.left = computeDayLeftPosition(day) - CURRENT_TIME_LINE_SIDE_BUFFER
                + 1;
        r.right = computeDayLeftPosition(day + 1)
                + CURRENT_TIME_LINE_SIDE_BUFFER + 1;
    ...
    private void drawSelectedRect(Rect r, Canvas canvas, Paint p) {
        ...
        if (mSelectionMode != SELECTION_HIDDEN && !mSelectionAllday) {
            ...
            if (mNumDays > 1) {
                ...
                int length = Math.min(mCellHeight, width) - NEW_EVENT_MARGIN
                        * 2;
                length = Math.min(length, NEW_EVENT_MAX_LENGTH);
                int verticalPadding = (mCellHeight - length) / 2;
                int horizontalPadding = (width - length) / 2;
                canvas.drawLine(r.left + horizontalPadding, midY, r.right
                        - horizontalPadding, midY, p);
                canvas.drawLine(midX, r.top + verticalPadding, midX, r.bottom
                        - verticalPadding, p);
            } else {
                ...
                canvas.drawText(mNewEventHintString, r.left
                        + EVENT_TEXT_LEFT_MARGIN,
    ...
    private void drawAllDayEvents(int firstDay, int numDays, Canvas canvas,
            Paint p) {
        ...
        int allDayEventClip = DAY_HEADER_HEIGHT + mAlldayHeight
                + ALLDAY_TOP_MARGIN;
        ...
        for (int i = 0; i < numEvents; i++) {
            ...
            if (mMaxAlldayEvents > mMaxUnexpandedAlldayEventCount) {
                ...
                if (event.top >= allDayEventClip) {
                    incrementSkipCount(mSkippedAlldayEvents, startIndex,
                            endIndex);
                    continue;
            ...
            Rect r = drawEventRect(event, canvas, p, eventTextPaint,
                    (int) event.top, (int) event.bottom);
            setupAllDayTextRect(r);
            StaticLayout layout = getEventLayout(mAllDayLayouts, i, event,
                    eventTextPaint, r);
    ...
    protected void drawMoreAlldayEvents(Canvas canvas, int remainingEvents,
            int day, Paint p) {
        int x = computeDayLeftPosition(day) + EVENT_ALL_DAY_TEXT_LEFT_MARGIN;
        int y = (int) (mAlldayHeight - .5f * MIN_UNEXPANDED_ALLDAY_EVENT_HEIGHT
                - .5f * EVENT_SQUARE_WIDTH + DAY_HEADER_HEIGHT + ALLDAY_TOP_MARGIN);
        Rect r = mRect;
        r.top = y;
        r.left = x;
        ...
        String text = mResources.getQuantityString(R.plurals.month_more_events,
                remainingEvents);
    ...
    private void drawEvents(int date, int dayIndex, int top, Canvas canvas,
        ...
        for (int i = 0; i < numEvents; i++) {
            ...
            if (date == mSelectionDay && !mSelectionAllday
            ...
            drawEventText(layout, r, canvas, mViewStartY + 4, mViewStartY
                    + mViewHeight - DAY_HEADER_HEIGHT - mAlldayHeight, false);
        }
        eventTextPaint.setAlpha(alpha);
    ...
    private Rect drawEventRect(Event event, Canvas canvas, Paint p,
        ...
        if (event == mClickedEvent) {
            ...
            color = Color.parseColor("#E64444");
//            color = mResources
//                    .getColor(R.color.event_rect_bg);
        }
    ...
    private void doSingleTapUp(MotionEvent ev) {
        ...
        if (mMaxAlldayEvents > mMaxUnexpandedAlldayEventCount) {
            ...
            if ((x < mHoursWidth && y > DAY_HEADER_HEIGHT && y < DAY_HEADER_HEIGHT
                    + mAlldayHeight)
                    || (!mShowAllAllDayEvents && mAnimateDayHeight == 0
                            && y < bottom && y >= bottom
        ...
        if (pressedSelected && mSavedClickedEvent == null) {
            ...
            mSelectionMode = SELECTION_SELECTED;
            mController.sendEventRelatedEventWithExtra(this,
                    EventType.CREATE_EVENT, -1, getSelectedTimeInMillis(), 0,
            ...
            /**
             * SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31
             * ,week view, date selection error when date between 1969-12-28 and
             * 1970-1-3
             *
             * @{
             */
            ...
            mSelectionMode = SELECTION_SELECTED;
            mController.sendEvent(this, EventType.GO_TO, startTime, endTime,
                    -1, ViewType.CURRENT, CalendarController.EXTRA_GOTO_TIME,
                    null, null);
        }
        invalidate();
    }
    ...
    private void doScroll(MotionEvent e1, MotionEvent e2, float deltaX,
        ...
        if (mTouchMode == TOUCH_MODE_DOWN) {
            ...
            /*
             * SPRD: bug408840 limit date between 1970/01/01 and 2037/12/31 @{
             * we keep vertical scrolling if not in date limited range
             */
            if (absDistanceX > absDistanceY
                    && !isOutOfLimitedDateRangeWhenMove(e1, e2)) {
    ...
    private void doFling(MotionEvent e1, MotionEvent e2, float velocityX,
        ...
        mScroller.fling(0 /* startX */, mViewStartY /* startY */,
                0 /* velocityX */, (int) -velocityY, 0 /* minX */,
                0 /* maxX */, 0 /* minY */, mMaxViewStartY /* maxY */,
    ...
    private class ContextMenuHandler implements
        ...
        public boolean onMenuItemClick(MenuItem item) {
            ...
            case MENU_EVENT_VIEW: {
                if (mSelectedEvent != null) {
                    mController.sendEventRelatedEvent(this,
                            EventType.VIEW_EVENT_DETAILS, mSelectedEvent.id,
            ...
            case MENU_EVENT_EDIT: {
                if (mSelectedEvent != null) {
                    mController.sendEventRelatedEvent(this,
                            EventType.EDIT_EVENT, mSelectedEvent.id,
                            mSelectedEvent.startMillis,
                            mSelectedEvent.endMillis, 0, 0, -1);
                }
                break;
            ...
            case MENU_DAY: {
                mController.sendEvent(this, EventType.GO_TO, getSelectedTime(),
                        null, -1, ViewType.DAY);
                break;
            }
            case MENU_AGENDA: {
            ...
            case MENU_EVENT_DELETE: {
                if (mSelectedEvent != null) {
                    ...
                    long id = selectedEvent.id;
                    mController.sendEventRelatedEvent(this,
                            EventType.DELETE_EVENT, id, begin, end, 0, 0, -1);
                }
                break;
            }
            default: {
    ...
    private void adjustNewSelectedDateTimeWithinLimitRange(Time newSelected) {
        ...
        int julianDay = Utils.getJulianDay(
                newSelected.toMillis(false /* useisDst */), newSelected.gmtoff);
    ...
    private boolean isOutOfLimitedDateRangeWhenMove(MotionEvent e1,
            MotionEvent e2) {
    ...
    /**
     * Fix bug 598011 Calendar display error after swipe screen and click
     * 'today' @
     **/
    public void setViewStartX(int x) {
        this.mViewStartX = x;
    }
    /** @} **/
}
